...
# "both" (default) keeps the historical behaviour: vLLM < 0.16.0 reads
# `reasoning_content`, >= 0.16.0 reads `reasoning`, and most servers ignore the
# key they do not know. Some servers instead treat the two as aliases of a single
# field and reject the pair outright -- Kimi-K3's Rust frontend answers
# `400 duplicate field 'reasoning'`. Set NEMO_GYM_REASONING_FIELD to
# "reasoning_content" or "reasoning" to emit exactly one.
